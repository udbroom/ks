...
| `spacing-<size>[-top\|-bottom][-static]`[^media-spacing] | Used with `media`: add one of Milo's shared spacing-scale classes (size runs `none`/`5xs`…`5xl`, e.g. `spacing-lg`, `spacing-xl-top`, `spacing-md-static-bottom`) to the block name to override its default top and/or bottom padding instead of the built-in per-breakpoint default. Omit `-top`/`-bottom` to set both sides; add `-static` for a fixed value instead of the normal viewport-scaled one. Recalculates automatically if the visitor resizes the window. | Only some deployments |
...
[^media-spacing]: [#6563](https://github.com/adobecom/milo/pull/6563) — Dušan Kosanović, 2026-08-26
