...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .block-item.active { background: #cbc9c9; }
  .block-item.active .block-title { color: #000; }
  ...
  /* Green = changed within the last 3 weeks, on both the footnote marker
...
<div id="app-wrapper">
  <div id="sidebar">
    ...
    <div id="block-list-wrapper">
      ...
      <div class="list-section-title">System blocks</div>
...
// each one is either hidden config (Section Metadata, Martech Metadata,
// Modal Metadata), site-wide chrome pulled from a separate shared doc rather
// than authored per page (Global Footer, Global Navigation), or a
// special-purpose embed/utility/system block tied to one specific context
// rather than general page content (Iframe embeds an external URL; Region
// Nav only lives inside the footer's language-picker modal; Modal is the
// dialog/fragment wrapper other blocks like Email Collection C2 render
// inside of, not something authored as its own row). Listed separately
// under "System blocks" so authors browsing for "a block to add to my page"
// aren't tripped up by ones they'd never place directly.
const NON_BLOCK_SLUGS = new Set([
  'global-footer', 'global-navigation', 'iframe', 'region-nav', 'section-metadata',
  'martech-metadata', 'modal-metadata', 'modal',
]);
...
function renderBlockGroup(containerEl, blocks, q) {
  containerEl.innerHTML = blocks.map(b => {
    ...
    // Small dot = changed within the last 3 weeks (see recentlyUpdated in
  ...
  mainEl.innerHTML = `
    ...
    <div class="doc-summary">
      ...
      <div class="doc-summary-footer">
        <span class="updated-date">Updated ${b.lastUpdated}${b.recentlyUpdated ? '<span class="recent-dot" title="Changed within the last 3 weeks"></span>' : ''}</span>
